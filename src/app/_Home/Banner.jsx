...
function Banner({ movies }) {
  ...
  const backGroundBanner = {
    backgroundImage: `url(${bannerMovie ? startLinkImag + bannerMovie.backdrop_path : ''})`,
          ...
          <div className="bannerContent     absolute bottom-[3rem] right-[2rem] left-[2rem] md:right-[7rem] md:left-[7rem]  ">
            ...
            <div className="title">
              ...
              <div className="typeMovie flex gap-1">
                {typeMovie.map((type, key) => <h3 key={key} className='text-neutral-400 border-primary text-[.7rem] border-b-[.03rem] '>{type}</h3>)}
              </div>
            </div>
           
            <div className={`rateAndButtons flex items-end justify-between `}>
               {/* rate movie */}
              <div className={`${mapStar.length > 3 ? 'w-[35%]' : ''} rate flex items-center gap-2 pt-[.8rem]`}>
                {mapStar.map((el) => <> <FaStar key={el} className='text-primary' />
                </>)}
              ...
              <div className="buttonWatchMovie flex  gap-1">
                <div className="WatchNow">
                  <Link className='transition-colors btn text-[.7rem] md:text-[.9rem] bg-primary font-medium hover:bg-hover' href={`/details/${bannerMovie.media_type}/${bannerMovie.id}`}>Watch Now</Link>
                </div>
                <div className="viewInfo">
                  <Link className='transition-colors text-[.7rem] md:text-[.9rem] btn border font-thin' href={`/details/${bannerMovie.media_type}/${bannerMovie.id}`}>More info</Link>
                </div>
              </div>
